...
d_stats <- df %>% split(.$dataset) %>% map(~ unique(select(.,sample, dataset, treatment, dataset_anon))) %>% map(~ cbind(unique(select(., dataset, treatment, dataset_anon)), samples = dim(.)[1])) %>% do.call(rbind,.) 
...
plot_gene_stats_anon <- function (d,gene_of_interest){
  d %>% filter(Gene==gene_of_interest) %>% select(scaled_value,dataset_anon,objective_response) %>% .[complete.cases(.),] %>% 
